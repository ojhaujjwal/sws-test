...
describe('Company', () => {
  ...
  describe('GET /api/v1/companies', () => {
    const createEndpoint = (queryString?: string) =>
      `${LIST_ENDPOINT}?${queryString}`;
    const sendGetReq = (queryString?: string) =>
      superTest.get(createEndpoint(queryString));

    it('should return all the companies in first page', async () => {
      const response = await sendGetReq().expect(200);
    ...
    it('should be able to alter the page size with a query param', async () => {
      const response1 = await sendGetReq('perPage=5').expect(200);
      ...
      const response2 = await sendGetReq('perPage=15').expect(200);
    ...
    it('should be able to filter company by specified exchange symbols', async () => {
      const response = await sendGetReq(
        'exchangeSymbols[]=NYSE&exchangeSymbols[]=ASX',
      ).expect(200);
      ...
      expect(
        new Set(response.body.items.map((item) => item.exchangeSymbol)),
      ).toEqual(new Set(['NYSE', 'ASX']));
    });

    it('should be able to filter by score', async () => {
      const response = await sendGetReq('scores[past]=2').expect(200);

      expect(response.body.items).toHaveLength(4);
      expect(response.body.meta.totalItems).toEqual(4);

      expect(
        new Set(response.body.items.map((item) => item.scores.past)),
      ).toEqual(new Set([2]));
    });

    it('should be able to filter by combination of filters in query', async () => {
      const response = await sendGetReq(
        'scores[past]=2&exchangeSymbols[]=NYSE&sort=-scores.value',
      ).expect(200);
      ...
      expect(response.body.links).toEqual({
        first: createEndpoint(
          'scores=&exchangeSymbols=NYSE&sort=-scores.value&limit=10',
        ),
        previous: '',
        next: '',
        last: createEndpoint(
          'scores=&exchangeSymbols=NYSE&sort=-scores.value&page=1&limit=10',
        ),
      });
    });

    it('should be able to sort in ascending/descending order by specified score', async () => {
      const response1 = await sendGetReq(
        'exchangeSymbols[]=ASX&sort=scores.value',
      ).expect(200);

      const scores1: ReadonlyArray<number> = response1.body.items.map(
        (company) => company.scores.value,
      );

      expect([...scores1].sort()).toEqual(scores1);

      const response2 = await sendGetReq(
        'exchangeSymbols[]=ASX&sort=-scores.value',
      ).expect(200);

      const scores2: ReadonlyArray<number> = response2.body.items.map(
        (company) => company.scores.value,
      );
    ...
    it('should be able to sort in ascending/descending order by volatility', async () => {
      ...
      const response1 = await sendGetReq(
        'exchangeSymbols[]=NasdaqGS&sort=volatility',
      ).expect(200);

      expect(response1.body.items.map((item) => item.name)).toEqual(
        volatilityOrder,
      );
      ...
      expect(response2.body.items.map((item) => item.name)).toEqual(
        volatilityOrder.reverse(),
      );
    });
  });
  ...
  describe('GET /api/v1/companies/:companyId/price-timeline', () => {
    const sendGetReq = (companyId: string, queryString?: string) =>
      superTest.get(
        `${LIST_ENDPOINT}/${companyId}/price-timeline?${queryString}`,
      );

    it('should return the prices', async () => {
      const response = await sendGetReq(
        '424EB65E-8C34-42BF-A107-61F93D4E9E6D',
      ).expect(200);
